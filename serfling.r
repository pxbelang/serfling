serfling <- read.csv('serflingSyn23.csv')
fluseason <- subset(serfling, WofY > 18 & WofY < 40)
serfreg <- lm(pctSyn3 ~ Week + I(Week*Week) + cos(2*pi*Week/52) + sin(2*pi*Week/52), data=fluseason)
...
write.csv(serftmp, file="c:\\temp\\serflingSyn2.csv")
